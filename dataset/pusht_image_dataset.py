...
class PushTPureImageDataset(BaseImageDataset):
    def __init__(self,
                 zarr_path,
                 transform=None,
                 horizon=1,
...
def test():
    ...
    zarr_path = os.path.expanduser(
        '~/PycharmProjects/diffusion_policy/diffusion_policy/data/pusht/pusht_cchi_v7_replay.zarr.zip')
    # print(zarr_path)
    # dataset = PushTImageDataset(zarr_path, horizon=16)
    store = zarr.ZipStore(zarr_path, mode='r')
    data = zarr.group(store=store)
    print(data.tree())
    print(data['data/n_contacts'][:])
