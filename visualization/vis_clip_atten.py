'''
    adapte from
    https://github.com/SeitaroShinagawa/CLIP-visualization/blob/main/visualize_clip_attentions.py
'''
...
parser = argparse.ArgumentParser(
    description="Visualizer of CLIP attention (average attention over heads) (option: visualize on each head, position embeddings)")
parser.add_argument("--index", default=0, type=int)
parser.add_argument("--dataset", default="place365", type=str)
parser.add_argument("--imgpath", default=None, type=str,
                    help="use an own image. If not None, this code ignore '--index' and '--dataset'")
parser.add_argument("--savedir", default="./", type=str)
parser.add_argument("--factor", default=1, type=int,
                    help="factor to enlarge input image size (makes position embedding resolution higher)")
...
with torch.no_grad():
    image_feat, image_attention_mh = model.encode_image_attention(image_input, return_attention=True, cond_attn=None,
                                                                  target_layer=args.layer_id)  # N, head, query, key
...
if args.unifyfigs:
    ...
    for query_id in range(attention_mean.shape[1]):
        ...
        ax.axis("off")
        im = ax.pcolor(attention_mean[0, query_id].reshape(7 * factor, 7 * factor),
        ...
        if args.visualize_eachhead:
            ...
            for head_id in range(n_head):
                ...
                ax.axis("off")
                im = ax.pcolor(image_attention_mh[0, head_id, query_id, 1:].reshape(7 * factor, 7 * factor),
                               norm=Normalize(vmin=0.0, vmax=args.vmax))
                ax.invert_yaxis()
                divider = make_axes_locatable(ax)
                cax = divider.append_axes("right", size="5%", pad=0.05)
