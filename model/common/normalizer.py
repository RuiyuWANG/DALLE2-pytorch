...
class LinearMaskNormalizer(DictOfTensorMixin):
    avaliable_modes = ['limits', 'gaussian']
    ...
    def _normalize_impl(self, x, forward=True, keypoint_dim=2):
        if isinstance(x, dict):
            result = dict()
            for key, value in x.items():
                if key == 'mask':
                    result[key] = torch.repeat_interleave(value, repeats=keypoint_dim, dim=-1)
                else:
                    params = self.params_dict[key]
                    result[key] = _normalize(value, params, forward=forward)
            return result
